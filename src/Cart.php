...
use Gloudemans\Shoppingcart\Exceptions\UndefinedConfigException;
...
class Cart
{
     ...
     */
    public function add($id, $name = null, $qty = null, $price = null, $weight = 0, $shipping = 0, $cuerrier_id = 0, array $options = [])
    {
        if ($this->isMulti($id)) {
        ...
        $cartItem = $this->createCartItem($id, $name, $qty, $price, $weight, $shipping, $cuerrier_id, $options);
    ...
    {
        $shipping = $this->getContent()->reduce(function ($total, CartItem $cartItem) {
            return $total + $cartItem->shipping;
        }, 0) / ($this->getContent()->count() == 0 ?: 1);
        if ($this->count() > 5){
            $totalShipping = $this->count() / 5;
            if ($this->count() % 5 == 0){
                $shipping = $shipping * $totalShipping;
            }else{
                $shipping = $shipping * ((int)$totalShipping + 1);
    ...
    public function shippingFloat()
    {
        return $this->getContent()->reduce(function ($shipping, CartItem $cartItem) {
            if((int)($this->count() / 5) > 0){
                if ($this->count() % 5 == 0){
                    return $cartItem->shipping * (int)($this->count() / 5)  ;
                }
                return $cartItem->shipping * ((int)($this->count() / 5) + 1);
            }

            return $cartItem->shipping;
        }, 0);
    }

    /**
     * Get the shipping charge of any order
     * @param int $decimals
     * @param string $decimalPoint
     * @param string $thousandSeperator
     */
    ...
    public function updateShipping($rowId, $shippingRate){
        $customerTable = config('shipping.customer_table');
        if ($customerTable != ''){
            $cartItem = $this->get($rowId);
            $this->shipping = $shippingRate;
            $cartItem->setShippingRate($shippingRate);
        }else{
            return UndefinedConfigException("Customer Table not set in shipping config.");
        }
     ...
     * @return \Gloudemans\Shoppingcart\CartItem
     */
    private function createCartItem($id, $name, $qty, $price, $weight, $shipping, $cuerrier_id, array $options)
    {
        if ($id instanceof Buyable) {
            ...
            $cartItem = CartItem::fromAttributes($id, $name, $price, $weight, $shipping, $cuerrier_id, $options);
    ...
    public function defaultShippingProvider(){
        $cart = $this->getContent()->first();
        return $cart->cuerrier_id;
    }
}
